...
# Install Qt 6.11.0
aqt install-qt mac ios 6.11.0 -O $HOME/qt -m all --autodesktop

# If the above fails on arm64, try:
arch -x86_64 aqt install-qt mac ios 6.11.0 -O $HOME/qt -m all --autodesktop
export PATH=$HOME/qt/6.11.0/ios/bin:$HOME/qt/6.11.0/macos/libexec:$HOME/qt/6.11.0/macos/bin:${PATH}
...
Hand-off mechanics (fork issue #13): the extension writes a payload into the shared App Group container `group.app.status.mobile` (the *pending intake slot*, file `pending-intake/share.json`) and wakes the host app via the unsupported responder-chain `openURL` workaround (`<host bundle id>://share-intake`, e.g. `app.status.mobile://share-intake` — the wake scheme is the host's bundle id so co-installed variants can't hijack each other's wake; the shared `status-app` scheme is claimed only by the release variant, fork issue #48). The host app reads, logs and clears the slot when the wake URL arrives — or, if the wake fails, on the next manual app open (degraded fallback, no data loss). The App Group is declared in both `Status.entitlements` and `Status-NoKeycard.entitlements`; iOS App Groups work with free personal teams too.
...
# arm host
aqt install-qt mac android 6.11.0 android_arm64_v8a -O $HOME/qt -m all --autodesktop
# x64 host
aqt install-qt mac android 6.11.0 android_x86_64 -O $HOME/qt -m all --autodesktop
# optional
aqt install-qt mac android 6.11.0 android_x86 -O $HOME/qt -m all
aqt install-qt mac android 6.11.0 android_armv7 -O $HOME/qt -m all
```
...
export PATH="$QTDIR/6.11.0/$QTTARGET/bin:$QTDIR/6.11.0/$QTTARGET/libexec:$QTDIR/6.11.0/$QTTARGET/bin:${PATH}"
